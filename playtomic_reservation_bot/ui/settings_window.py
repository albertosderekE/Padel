...
class SettingsWindow(ctk.CTkToplevel):
    ...
    def save_tz(self) -> None:
        try:
            local_value = self.local_tz.get().strip()
            target_value = self.target_tz.get().strip()
            load_timezone(local_value)
            load_timezone(target_value)
            self._upsert_setting("local_tz", local_value)
            self._upsert_setting("target_tz", target_value)
            self.on_change()
